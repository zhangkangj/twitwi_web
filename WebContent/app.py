...
def ivoted_tweet():
    ...
    if time == None:
        ...
        cursor.execute("""SELECT id,created_at,name,screen_name,text, state, entity FROM election_realtime WHERE created_at <= %s and created_at > %s - 60 AND !isnull(state) LIMIT 15""", (time, time))
    entry = cursor.fetchone()
    while entry:
        id = str(entry[0])
        name = entry[2].decode("utf-8",errors='ignore') 
