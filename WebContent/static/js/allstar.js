...
function hover_state(d) {
	...
	if (d) {
		...
		// calculate stats
		var east_count  = mention_json[d.properties.abbreviation][0];
		var west_count = mention_json[d.properties.abbreviation][1];
		$('#east-count').text(east_count);
		$('#west-count').text(west_count);
		move_box();
		box.show();
	}
...
function update_tweet(){
	...
	d3.json("/nba_tweet.json", function(json){
		for (i in json){
...
function show_tweet(i, json){
	...
	if (state != 'US') {
		//flash = (entity=='west') ? 'flash-red' : 'flash-blue';
...
$(document).ready(function() {
	d3.json("/static/dat/us_states.json", function(json){
		map_json = json;
		d3.json("/nba.json", function(json){
			mention_json = json;
			setup_map();
